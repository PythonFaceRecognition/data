...
def align(output_image_folder, q):
    sp = dlib.shape_predictor(q)
    detector = dlib.get_frontal_face_detector()
    items = os.listdir(output_image_folder)
    for i in range(1,len(items)+1):
        for (root, dirs, files) in os.walk(output_image_folder+"\\"+str(i)):
            for f in files:
                ...
                for det in dets:
                    ...
                    for image in images:
                        ...
                        cv2.imwrite(output_image_folder+"\\"+str(i)+"\\"+ str(f),cv_bgr_image)
        ...
        while (A1 < 13):
            ...
            while(A2<A2+1):
                ...
                for i, d in enumerate(face_rects):
                    ...
                    cv2.imwrite(folder+"\\" + str(A) + "\\" + str(A1) + "_" + str(A2) + ".png", crop_img)
                A2 += 1
            A1 += 1
        A += 1